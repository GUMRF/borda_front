...
export default function TaskCard(props) {
	...
	return (
		<>
			...
			>
				<div
					className={`${isOpen ? `border-b-2 border-${props.color}-300` : ""}`}
					...
					>
						<div className="">
							<div className="flex flex-row">
								<span className="text-lime-500 font-bold text-xl">{props.category}</span>
								{isOpen ? (
									<div className="flex flex-row">
										{props.tags.map((tag) => (
											<div className="text-black font-semibold text-xl ml-5">#{tag}</div>
										))}
									</div>
								) : null}
							</div>
